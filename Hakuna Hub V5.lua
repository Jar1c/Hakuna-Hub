...
local secMovement = TabMain:AddSection("Movement", "solar/running-bold")
...
-- ---- FLY ----
local secFly = TabMain:AddSection("Fly", "solar/wing-bold")
...
-- Fly system with spam-proof toggle
local flyBusy = false
...
function StartFly()
    ...
    if State.isFlyNoclip then startNoclip() end
...
function StopFly()
    ...
    if State.isFlyNoclip then stopNoclip() end
...
secFly:AddToggle("FlyNoClipToggle", {
    Title    = "Fly NoClip",
    Icon     = "solar/ghost-bold",
    Default  = false,
    Callback = function(v)
        State.isFlyNoclip = v
        if v and State.isFlying then
            startNoclip()
        elseif not v then
            stopNoclip()
        end
...
-- ---- NOCLIP ----
local secNoclip = TabMain:AddSection("NoClip", "solar/ghost-bold")

local noclipCachedParts = {}
local noclipConnection = nil

local function refreshNoclipParts()
    table.clear(noclipCachedParts)
    local char = LocalPlayer.Character
    if not char then return end
    for _, p in ipairs(char:GetChildren()) do
        if p:IsA("BasePart") then
            table.insert(noclipCachedParts, p)
        elseif p:IsA("Accessory") then
            local handle = p:FindFirstChild("Handle")
            if handle and handle:IsA("BasePart") then
                table.insert(noclipCachedParts, handle)
            end
        end
    end
end
...
startNoclip = function()
    ...
    State.connections["selfNoClipCharAdded"] = LocalPlayer.CharacterAdded:Connect(function()
        task.wait(0.1)
        refreshNoclipParts()
    end)
end
...
-- ---- ANTI-FLING ----
local secAntiFling = TabMain:AddSection("Anti-Fling", "solar/shield-check-bold")
...
-- ---- ESP ----
local secESP = TabMain:AddSection("ESP", "solar/eye-bold")
...
TabPlayers:AddSection("Players List", "solar/users-bold")
...
-- =================== TELEPORT TAB ===================
local secWaypoint = TabTeleport:AddSection("Waypoint", "solar/map-pin-bold")
...
local secClickTP = TabTeleport:AddSection("Click Teleport", "solar/cursor-bold")
...
local secAutoReturn = TabTeleport:AddSection("Auto Return", "solar/refresh-bold")
...
local secSafeZone = TabTeleport:AddSection("Safe Zone", "solar/shield-bold")
...
-- =================== SETTINGS TAB ===================
local secDisplay = TabSettings:AddSection("Display", "solar/monitor-bold")
...
local secButtons = TabSettings:AddSection("Buttons", "solar/menu-bold")
...
local secExit = TabSettings:AddSection("Exit", "solar/logout-bold")
secExit:AddButton({
...
secExit:AddButton({
    Title    = "Reset UI Settings (Fix Empty UI)",
    Icon     = "solar/refresh-bold",
    Callback = function()
        pcall(function()
            if InterfaceManager and InterfaceManager.Reset then
                InterfaceManager:Reset()
            end
        end)
        pcall(function()
            if SaveManager and SaveManager.Reset then
                SaveManager:Reset()
            end
        end)
        pcall(function() Window:Destroy() end)
        _G.HakunaHubLoaded = nil
        Fluent = nil
        Notify("Reset", "UI settings cleared. Re-execute the script.", 5)
    end,
})
secExit:AddDivider()

local secAntiAFK = TabSettings:AddSection("Anti-AFK", "solar/cup-bold")
...
local secKeybinds2 = TabSettings:AddSection("Keybinds", "solar/keyboard-bold")
...
-- Initialize managers safely; if any fail, still show the window
local managersOk = true

local function safeCall(name, fn)
    local ok, err = pcall(fn)
    if not ok then
        warn("[Hakuna Hub] " .. name .. " failed: " .. tostring(err))
        managersOk = false
    end
end

safeCall("MediaManager", function()
    MediaManager:SetFolder("FluentShowcase/MediaCache")
end)

safeCall("InterfaceManager", function()
    InterfaceManager:SetLibrary(Fluent)
    InterfaceManager:SetFolder("FluentShowcase")
    InterfaceManager:BuildInterfaceSection(TabSettings)
end)

-- Load settings separately; corrupted saved themes can make content invisible
safeCall("InterfaceManager:LoadSettings", function()
    InterfaceManager:LoadSettings()
end)

-- GUARANTEE the window is always shown regardless of manager errors
pcall(function() Window:Show() end)

safeCall("SaveManager", function()
...
safeCall("FloatingButtonManager", function()
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == State.flyKey then
        -- Direct fly toggle - skip UI to avoid desync on spam
        if flyBusy then return end
        if State.isFlying then
            StopFly()
            pcall(function() State.flyToggleObj:SetValue(false) end)
        else
            StartFly()
            pcall(function() State.flyToggleObj:SetValue(true) end)
        end
        return
    end
...
-- =================== FLUENT UI PATCHES ===================
-- Fixes: (1) save icon color on light themes, (2) title bar "Desktop PC" spacing
local function PatchFluentUI()
    task.wait(1)
    pcall(function()
        local root = Window.Root
        if not root then return end

        -- Get current theme text color for adaptive icon coloring
        local themeTextColor = Color3.fromRGB(230, 245, 255) -- default dark theme
        pcall(function()
            -- Try multiple ways to get the current theme's text color
            local Creator = Fluent.Creator
            if Creator and Creator.GetThemeProperty then
                local tc = Creator.GetThemeProperty("Text")
                if tc then themeTextColor = tc end
            end
        end)

        for _, desc in ipairs(root:GetDescendants()) do
            -- Fix 1: Save icon color - force ImageColor3 on all _ElemIcon elements
            if desc.Name == "_ElemIcon" and desc:IsA("ImageLabel") then
                desc.ImageColor3 = themeTextColor
            end

            -- Fix 2: Title bar "Desktop PC" spacing - push badge further from minimize
            if desc.Name == "_DeviceBadge" and desc:IsA("Frame") then
                local isMobile = UserInputService.TouchEnabled
                local spacing = isMobile and 10 or 18
                desc.Position = UDim2.new(1, -(168 + spacing), 0, 9)
            end
        end
    end)
end
...
task.delay(0.5, function()
    Window:SelectTab(1)
    task.spawn(PatchFluentUI)

    -- Re-patch icons when theme changes
    pcall(function()
        local Creator = Fluent.Creator
        if Creator and Creator.ThemeChanged then
            Creator.ThemeChanged:Connect(function()
                task.wait(0.3)
                PatchFluentUI()
            end)
        end
...
task.spawn(function()
    ...
    if success and err then
        ...
        if not ok then
            warn("[Hakuna Hub] Init failed: " .. tostring(runErr))
        end
    else
        warn("[Hakuna Hub] Fluent load failed: " .. tostring(err))
    end
end)
